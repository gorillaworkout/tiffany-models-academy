...
export async function POST(req: Request) {
  try {
    ...
    const users = await d1Query('SELECT id, batch_id, email, nama_lengkap as name, role, status, alamat, ukuran_heels, ukuran_baju, ebook_package_id FROM member WHERE email = ? AND password = ?', [data.email, data.password]);
